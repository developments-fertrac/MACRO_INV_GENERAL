        ...
        try:
            ...
            if ws_lp:
                ...
                if ref_fertrac_col:
                    ...
                    # Leer todas las filas de datos de INV LISTA PRECIOS
                    log(f"  📖 Leyendo {last_row_lp - HEADER_ROW_LP} filas de INV LISTA PRECIOS...")
                    filas_lp = []
                    for row_idx in range(start_data_row_lp, last_row_lp + 1):
                        fila = []
                        for col_idx in range(1, last_col + 1):
                            val = ws_lp.Cells(row_idx, col_idx).Value
                            fila.append(val)
                        filas_lp.append(fila)
                    ...
                    # Escribir las filas ordenadas de vuelta a Excel
                    log(f"  ✍️ Escribiendo {len(filas_ordenadas)} filas reordenadas...")
                    for i, fila in enumerate(filas_ordenadas):
                        row_idx = start_data_row_lp + i
                        for col_idx, valor in enumerate(fila, start=1):
                            ws_lp.Cells(row_idx, col_idx).Value = valor
