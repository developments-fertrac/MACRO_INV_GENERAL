...
def com_convert_to_xlsx(path: Path, passwords: list[str] | None = None) -> Path:
    ...
    try:
        ...
        if not tmp.exists():
            raise RuntimeError(f"El archivo temporal no se creó: {tmp}")
        ...
        # Eliminar archivo temporal si se creó parcialmente
        if tmp.exists():
            try:
                tmp.unlink()
            except:
        ...
        raise RuntimeError(f"Error al convertir '{path.name}': {e}")
